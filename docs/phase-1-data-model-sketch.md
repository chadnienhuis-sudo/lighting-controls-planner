...
4. **Hard-coded space-type defaults table: seed from a generic template; users adjust per project.** Resolved 2026-04-23. Ship a conservative generic baseline (dual-tech occupancy sensor, 10-minute timeout, 100% occupied, manual on/off + dimming where applicable) across categories, with a small handful of category-level deviations where the code itself forces different behavior (stairwells bilevel, corridors no dimming, storage PIR, etc.). Users override in the group editor when the project demands it. Trade-off: the out-of-box output is less opinionated than a curated list, which weakens the "design-aid encoding A+ taste" story somewhat — mitigated later when the A+-customer-scoped defaults table (`customer_space_type_defaults` in backend-migration-plan.md §5) lets A+ ship its own opinionated layer on top of the generic baseline without baking it into the free tier.
