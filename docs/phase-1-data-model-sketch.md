...
3. **Narrative text rewrite scope:** does weaving resolved setpoint values into the sequence paragraph belong in Phase 1, or Phase 3? If Phase 1, narrative.ts changes materially. If deferred, the submittal has a "Recommended settings" block + a paragraph that doesn't reference specific numbers — slight redundancy but lower risk.
4. **Hard-coded space-type defaults table:** do we ship a reviewed-by-Chad list, or seed from a generic template and ask the user to adjust per project? First approach is better for the design-aid positioning, but needs a curation pass.
